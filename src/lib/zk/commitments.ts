...
/**
 * Simple synchronous hash function using a custom implementation
 * Based on the djb2 algorithm extended to produce 256-bit output
 */
function simpleHash256(data: Uint8Array): Uint8Array {
  // Use multiple hash rounds to create 256-bit output
...
// Large prime for modular arithmetic (secp256k1 order)
...
export function createBalanceCommitment(balanceLamports: number): PedersenCommitment {
  const value = BigInt(Math.floor(Math.max(0, balanceLamports)));
  ...
  const commitmentHash = simpleHash256(commitmentBytes);
...
export function verifyCommitment(
  ...
  const value = BigInt(Math.floor(Math.max(0, claimedBalance)));
  ...
  // Hash and compare
  const recomputedHex = recomputed.toString(16).padStart(64, '0');
  const recomputedBytes = hexToBytes(recomputedHex);
  const recomputedHash = bytesToHex(simpleHash256(recomputedBytes));
...
export function createRangeProof(
  ...
  const proofHash = bytesToHex(simpleHash256(proofData));
...
export function createEqualityProof(
  ...
  return {
    ...
    proofHash: bytesToHex(simpleHash256(proofData)),
    timestamp: Date.now(),
  };
}
...
export function hashPublicKey(publicKey: string): string {
  ...
  return bytesToHex(simpleHash256(data));
}
